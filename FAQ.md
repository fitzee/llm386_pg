...
Tool *results* are different and absolutely belong in memory. Store each one as a `ToolResult` block and tie it to the assistant turn that produced it with a typed edge: `add_edge(assistant_message_id, tool_result_id, kind="tool-invocation")`. Edge-aware paging keeps the call/result paired — when the assistant turn is in the working set (it usually is, being recent), the expansion pass co-retrieves its tool result even if the result fell outside its section budget. The chat packer then renders results with the `tool` role so the model sees them in the right slot. This is the path that most MCP-driven agents care about: the model called `read_file`, the result came back as 4KB of code, you commit it once, and every subsequent turn benefits from the dedup, summarization, and budget-aware packing. (`ToolInvocation` is on by default; if you instead want the reverse — pulling the assistant turn back in when only the *result* was retrieved — set `follow_provenance_parents` in the `[edges]` config and store the result with `parents=[assistant_message_id]`.)
